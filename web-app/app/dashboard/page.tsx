    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                // Fetch worksheets based on user role
                if (user?.role === 'TEACHER') {
                    // For teachers, we'd need to fetch their classes first, then worksheets
                    ...
                    // For students, fetch their worksheets
                    const worksheetsData = await worksheetAPI.getWorksheetsByStudent(user.id);
                    setWorksheets(worksheetsData);
                } else if (user?.role === 'ADMIN' || user?.role === 'SUPERADMIN') {
                    // For admins, we could show recent worksheets across all classes
                    // This is a simplified version
                    const worksheetsData = await worksheetAPI.getWorksheetsByClass('1025d255-a661-4a56-9876-7eac665c6ee1');
                    setWorksheets(worksheetsData.slice(0, 5)); // Show only 5 most recent
                }
